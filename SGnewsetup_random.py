...
# -*- coding: utf-8 -*-
"""
Created on Mon Dec  7 21:39:28 2020
...
def impute_data(df):
    imputer_iterative = IterativeImputer(missing_values=np.NaN, random_state=42,initial_strategy='most_frequent', max_iter=100)
...
def forward_fs_lr(Xitrain, Xval, yitrain, yval,best_features, all_snps,
    ...
    for feat in all_snps:
        ...
        for k in ParameterGrid(param_grid):
            pipe = LogisticRegression(random_state=42, max_iter=10000).set_params(**k)
...
def fs_ml_random(data_x, data_y, clinical_features, max_features, seed, n_outer=5, n_inner=5, index_col_name='Unnamed: 0', data_x_path="/home/proj
ects/pr_46457/people/sarga/neurotox/data/X_data.csv", function=forward_fs_rf, imputation=True, normalize=False):
    print("seed: ",seed)
    ...
    while no_features < max_features:
        ...
        for i, (train_idx, test_idx) in enumerate(outer_cv.split(data_x, data_y)):
            ...
            for itrain_idx, val_idx in inner_cv.split(X_train, y_train):
                ...
                all_snps=return_random_lines_file(30, seed)
...
#forward selection_machine learning
def check_last_performaces(data_x, data_y, clinical_features,max_features, seed, n_outer=5, n_inner=5, index_col_name='Unnamed: 0', data_x_path="/
home/projects/pr_46457/people/sarga/neurotox/data/X_data.csv", function=forward_fs_rf, imputation=True, normalize=False, parameters={'n_estimators
': [10, 50, 100, 150, 200]}, model=RandomForestClassifier(random_state=42)):
    print("seed: ",seed)
    ...
    y_test_pred={}
    y_test_index={}
    fields=fs_ml_random(data_x, data_y, clinical_features,max_features, seed ,n_outer, n_inner, index_col_name, data_x_path, function, imputation,
 normalize)
...
results=Parallel(n_jobs=32)(delayed(check_last_performaces)(X, y, clinical_features, len(clinical_features)+30, seed, function=forward_fs_lr, norm
alize=True,parameters={'C' : np.logspace(-4, 4, 20)}, model=LogisticRegression(random_state=42, max_iter=10000, multi_class='auto')) for seed in r
ange(1,31))
...
results_roc_auc_pd.to_csv("/home/projects/pr_46457/people/sarga/neurotox/results/forward_select_training/setup4_final/NTX6/pheno1/fs_lr_30seeds_cl
inical_snps_allRANDOM.txt")
